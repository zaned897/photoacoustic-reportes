...
def main():
    ...
    ax[0].set_ylabel("tensión en el láser (V)")
    ax[0].set_title("Osciloscopio en el láser (tensión) — 200 MS/s", color=BLUE, fontsize=10, loc="left")
    ...
    ax[1].set_title("DAQ en el sense de corriente 0.1 Ω — 54 MS/s, 12 bit", color=ORANGE, fontsize=10, loc="left")
    for a in ax:
        a.grid(alpha=0.3)
        a.axvline(0, color="0.6", ls=":", lw=1)
    ax[1].set_xlim(-0.6, 4.8)                     # ventana del DAQ
    fig.suptitle("Mismo disparo: tensión en el láser (osc.) vs corriente del diodo (DAQ)", fontsize=11)
    fig.tight_layout()
    fig.savefig(OUT, dpi=150)
    print("OK ->", OUT)
